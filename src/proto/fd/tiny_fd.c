...
{
    tiny_fd_frame_info_t *slot =
        tiny_fd_queue_allocate(&handle->frames.s_queue, type, ((const uint8_t *)data) + 2, len - 2);
...
static void __confirm_sent_frames(tiny_fd_handle_t handle, uint8_t peer, uint8_t nr)
{
    ...
    while ( nr != handle->peers[peer].confirm_ns )
    {
        ...
        tiny_fd_frame_info_t *slot = tiny_fd_queue_get_next(&handle->frames.i_queue, TINY_FD_QUEUE_I_FRAME, address,
                                                            handle->peers[peer].confirm_ns);
        if ( slot != NULL )
        {
            if ( handle->on_send_cb )
            {
                ...
                handle->on_send_cb(handle->user_data,
                                   __is_primary_station(handle) ? (__peer_to_address_field(handle, peer) >> 2)
...
static void on_frame_read(void *user_data, uint8_t *data, int len)
{
    ...
    else if ( handle->peers[peer].state != TINY_FD_STATE_CONNECTED &&
              handle->peers[peer].state != TINY_FD_STATE_DISCONNECTING )
    {
        ...
        tiny_frame_header_t frame = {
            .address = __peer_to_address_field(handle, peer) | HDLC_CR_BIT,
            .control = (handle->mode == TINY_FD_MODE_NRM ? HDLC_U_FRAME_TYPE_SNRM : HDLC_U_FRAME_TYPE_SABM) |
                       HDLC_U_FRAME_BITS,
        };
        __put_u_s_frame_to_tx_queue(handle, TINY_FD_QUEUE_U_FRAME, &frame, 2);
...
int tiny_fd_init(tiny_fd_handle_t *handle, tiny_fd_init_t *init)
{
    ...
    *handle = NULL;
    ...
    // TODO: Hack: remove - 4
    int hdlc_ll_size =
        (int)((uint8_t *)init->buffer + init->buffer_size - ptr - 4 - // Remaining size
              init->window_frames * // Number of frames multiply by frame size (headers + payload + pointers)
                  (sizeof(tiny_fd_frame_info_t *) + init->mtu + sizeof(tiny_fd_frame_info_t) -
                   sizeof(((tiny_fd_frame_info_t *)0)->payload)) -
              TINY_FD_U_QUEUE_MAX_SIZE * (sizeof(tiny_fd_frame_info_t *) + sizeof(tiny_fd_frame_info_t)) -
    ...
    int queue_size =
        tiny_fd_queue_init(&protocol->frames.i_queue, ptr, (int)((uint8_t *)init->buffer + init->buffer_size - ptr),
    ...
    ptr = TINY_ALIGN_BUFFER(ptr);
    queue_size =
        tiny_fd_queue_init(&protocol->frames.s_queue, ptr, (int)((uint8_t *)init->buffer + init->buffer_size - ptr),
    ...
    tiny_events_set(&protocol->events,
                    FD_EVENT_QUEUE_HAS_FREE_SLOTS | (__is_primary_station(protocol) ? FD_EVENT_HAS_MARKER : 0));
    *handle = protocol;
...
{
    uint8_t *data = NULL;
    // LOG(TINY_LOG_DEB, "[%p] QUEUE SEARCH: [%02X] [%02X]\n", handle, address, TINY_FD_QUEUE_S_FRAME |
    // TINY_FD_QUEUE_U_FRAME);
    tiny_fd_frame_info_t *ptr =
        tiny_fd_queue_get_next(&handle->frames.s_queue, TINY_FD_QUEUE_S_FRAME | TINY_FD_QUEUE_U_FRAME, address, 0);
    if ( ptr != NULL )
    {
        ...
        *len = ptr->len + sizeof(tiny_frame_header_t);
        ...
        if ( (data[1] & HDLC_U_FRAME_MASK) == HDLC_U_FRAME_BITS )
        {
            LOG(TINY_LOG_INFO, "[%p] Sending U-Frame type=%02X with address [%02X] to %s\n", handle,
                data[1] & HDLC_U_FRAME_TYPE_MASK, data[0], __is_primary_station(handle) ? "secondary" : "primary");
        }
        else if ( (data[1] & HDLC_S_FRAME_MASK) == HDLC_S_FRAME_BITS )
        {
            LOG(TINY_LOG_INFO, "[%p] Sending S-Frame N(R)=%02X, type=%s with address [%02X] to %s\n", handle,
                data[1] >> 5, ((data[1] >> 2) & 0x03) == 0x00 ? "RR" : "REJ", data[0],
    ...
    if ( ptr != NULL )
    {
        ...
        *len = ptr->len + sizeof(tiny_frame_header_t);
        LOG(TINY_LOG_INFO, "[%p] Sending I-Frame N(R)=%02X,N(S)=%02X with address [%02X] to %s\n", handle,
            handle->peers[peer].next_nr, handle->peers[peer].next_ns, data[0],
            __is_primary_station(handle) ? "secondary" : "primary");
...
static uint8_t *tiny_fd_get_next_frame_to_send(tiny_fd_handle_t handle, int *len, uint8_t peer)
{
    ...
    if ( data == NULL && handle->mode == TINY_FD_MODE_NRM )
    {
        ...
        if ( __is_primary_station(handle) && (handle->peers[peer].state == TINY_FD_STATE_DISCONNECTED ||
                                              handle->peers[peer].state == TINY_FD_STATE_CONNECTING) )
        {
            tiny_frame_header_t frame = {
                .address = address,
...
int tiny_fd_get_tx_data(tiny_fd_handle_t handle, void *data, int len, uint32_t timeout)
{
    ...
    while ( result < len )
    {
        ...
        else
        {
            ...
            {
                if ( tiny_events_wait(&handle->events, FD_EVENT_TX_DATA_AVAILABLE, EVENT_BITS_CLEAR, timeout) ||
                     handle->mode == TINY_FD_MODE_NRM )
                {
                    int frame_len = 0;
...
int tiny_fd_buffer_size_by_mtu_ex(uint8_t peers_count, int mtu, int tx_window, hdlc_crc_t crc_type, int rx_window)
{
    ...
    return sizeof(tiny_fd_data_t) + TINY_ALIGN_STRUCT_VALUE - 1 + peers_count * sizeof(tiny_fd_peer_info_t) +
           // RX side
...
int tiny_fd_send_to(tiny_fd_handle_t handle, uint8_t address, const void *data, int len, uint32_t timeout)
{
    ...
    while ( left > 0 )
    {
        int size = left < tiny_fd_queue_get_mtu(&handle->frames.i_queue)
                       ? left
                       : tiny_fd_queue_get_mtu(&handle->frames.i_queue);
